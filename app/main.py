...
def predict(data: request_body):
    # Preparar features
    input_features = [[data.Size, data.Juiciness, data.Ripeness, data.Acidity]]

    # Classificar Fruta
    y_pred = modelo_qualidade.predict(input_features)[0].astype(int)
    y_prob = modelo_qualidade.predict_proba(input_features)[0].atype(float)
